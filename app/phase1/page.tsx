...
export default function Phase1() {
  ...
  useEffect(() => {
    if (p1?.draft && !p1.lockedAt) {
      try { setSpec(JSON.parse(p1.draft) as Phase1Spec); } catch {}
    }
  }, [p1]);
  ...
  const toggleSlider = (key: SliderKey) => {
    ...
    if (set.has(key)) {
      ...
      if (set.size >= 2) return; // max 2 sliders
      set.add(key);
    }
  ...
  const series = useMemo(() => {
    const n = Math.max(1, Math.min(720, Math.round(spec.horizon))); // bornes raisonnables
    const arr: number[] = new Array(n + 1);
    arr[0] = isFinite(spec.initialStockValue) ? spec.initialStockValue : 0;
    const inflow = Math.max(0, isFinite(spec.inflowValue) ? spec.inflowValue : 0);
    const outflow = Math.max(0, isFinite(spec.outflowValue) ? spec.outflowValue : 0);
    for (let t = 1; t <= n; t++) {
      arr[t] = arr[t - 1] + (inflow - outflow);
    }
    return arr;
        ...
        {locked ? (
          ...
          <div className="space-y-6">
            ...
            <section className="space-y-3">
              ...
              <div className="grid gap-3">
                <label className="block">
                  <span className="text-sm">Nom du stock</span>
                  <input className="mt-1 w-full border rounded-lg p-2" placeholder="Ex. Trésorerie"
                    value={spec.stockName} onChange={e => update({ stockName: e.target.value })} />
                </label>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <label className="block">
                    <span className="text-sm">Unité du stock</span>
                    <input className="mt-1 w-full border rounded-lg p-2" placeholder="Ex. euros, habitants"
                      value={spec.stockUnit} onChange={e => update({ stockUnit: e.target.value })} />
                  </label>
                  <label className="block">
                    <span className="text-sm">Unité de temps (pas du modèle)</span>
                    <select className="mt-1 w-full border rounded-lg p-2"
                      value={spec.timeUnit} onChange={e => update({ timeUnit: e.target.value as any })}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <label className="block">
                    ...
                    <input className="mt-1 w-full border rounded-lg p-2" placeholder="Ex. Recettes"
                      value={spec.inflowName} onChange={e => update({ inflowName: e.target.value })} />
                  </label>
                  <label className="block">
                    <span className="text-sm">Nom du flux de sortie</span>
                    <input className="mt-1 w-full border rounded-lg p-2" placeholder="Ex. Dépenses"
                      value={spec.outflowName} onChange={e => update({ outflowName: e.target.value })} />
                  </label>
                </div>

                <label className="block">
                  <span className="text-sm">Nom du stock de départ</span>
                  <input className="mt-1 w-full border rounded-lg p-2" placeholder="Ex. Trésorerie initiale"
                    value={spec.initialStockName} onChange={e => update({ initialStockName: e.target.value })} />
                </label>
              </div>
            </section>
            ...
            <section className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <NumberField
                  label={`Valeur du stock de départ (${spec.initialStockName || "Stock de départ"})`}
                  value={spec.initialStockValue}
                  onChange={v => update({ initialStockValue: v })}
                />
                <NumberField
                  label={`Flux d’entrée constant (${spec.inflowName || "Entrée"})`}
                  value={spec.inflowValue}
                  min={0}
                  onChange={v => update({ inflowValue: Math.max(0, v) })}
                />
                <NumberField
                  label={`Flux de sortie constant (${spec.outflowName || "Sortie"})`}
                  value={spec.outflowValue}
                  min={0}
                  onChange={v => update({ outflowValue: Math.max(0, v) })}
                />
                <NumberField
                  label={`Horizon (nombre de ${spec.timeUnit}s)`}
                  value={spec.horizon}
                  min={1}
                  max={720}
                  step={1}
                  onChange={v => update({ horizon: Math.max(1, Math.min(720, Math.round(v))) })}
                />
              </div>
            ...
            {/* 3) Choix des sliders (max 2) */}
            <section className="space-y-2">
              <h2 className="text-lg font-medium">Sliders (optionnels, max 2)</h2>
              <div className="flex flex-wrap gap-3 text-sm">
                <label className="inline-flex items-center gap-2">
                  <input type="checkbox" checked={spec.sliderKeys.includes("inflow")} onChange={() => toggleSlider("inflow")} />
                  {spec.inflowName || "Flux d’entrée"}
                </label>
                <label className="inline-flex items-center gap-2">
                  <input type="checkbox" checked={spec.sliderKeys.includes("outflow")} onChange={() => toggleSlider("outflow")} />
                  {spec.outflowName || "Flux de sortie"}
                </label>
                <label className="inline-flex items-center gap-2">
                  <input type="checkbox" checked={spec.sliderKeys.includes("initial")} onChange={() => toggleSlider("initial")} />
                  {spec.initialStockName || "Stock de départ"}
                </label>
              </div>

              {/* Sliders visibles selon la sélection */}
              <div className="grid gap-3">
                {spec.sliderKeys.includes("inflow") && (
                  <RangeField
                    label={`Slider ${spec.inflowName || "Entrée"} (${spec.derivedFlowUnit || "—"})`}
                    value={spec.inflowValue}
                    min={0}
                    max={bestMax(spec.inflowValue)}
                    step={Math.max(1, Math.round(bestMax(spec.inflowValue) / 50))}
                    onChange={v => update({ inflowValue: v })}
                  />
                )}
                {spec.sliderKeys.includes("outflow") && (
                  <RangeField
                    label={`Slider ${spec.outflowName || "Sortie"} (${spec.derivedFlowUnit || "—"})`}
                    value={spec.outflowValue}
                    min={0}
                    max={bestMax(spec.outflowValue)}
                    step={Math.max(1, Math.round(bestMax(spec.outflowValue) / 50))}
                    onChange={v => update({ outflowValue: v })}
                  />
                )}
                {spec.sliderKeys.includes("initial") && (
                  <RangeField
                    label={`Slider ${spec.initialStockName || "Stock de départ"} (${spec.derivedStockUnit || "—"})`}
                    value={spec.initialStockValue}
                    min={0}
                    max={bestMax(spec.initialStockValue)}
                    step={Math.max(1, Math.round(bestMax(spec.initialStockValue) / 50))}
                    onChange={v => update({ initialStockValue: v })}
                  />
                )}
              </div>
...
function bestMax(v: number) {
  const base = Math.abs(v) || 10;
  const m = Math.pow(10, Math.floor(Math.log10(base)));
  const k = Math.ceil(base / m);
  return (k * m) * 2; // un peu au-dessus
}

/* Mini graphe SVG — simple et lisible */
function MiniChart({ series, unitY, unitX }: { series: number[]; unitY: string; unitX: string; }) {
  ...
  const toX = (i: number) => PAD + (i * (W - 2 * PAD)) / (series.length - 1 || 1);
...
function LockedView({ content, onBack }: { content?: string; onBack: () => void; }) {
  ...
  try { spec = content ? (JSON.parse(content) as Phase1Spec) : null; } catch {}
  return (
    <div className="space-y-4">
      ...
      {spec ? (
        <>
          ...
          {/* On reconstitue la série pour la lecture seule */}
...
function ReadOnlyChart({ spec }: { spec: Phase1Spec }) {
  const series = useMemo(() => {
    const n = Math.max(1, Math.min(720, Math.round(spec.horizon)));
    const arr: number[] = new Array(n + 1);
    arr[0] = spec.initialStockValue;
    for (let t = 1; t <= n; t++) {
      arr[t] = arr[t-1] + (spec.inflowValue - spec.outflowValue);
    }
    return arr;
  }, [spec]);
