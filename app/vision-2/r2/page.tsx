...
type Row = {
  ...
  fin: number;
  ecartObj: number; // Objectif - stock_debut(t)
};
...
function buildTable(s0: number, enc: number, dec: number, h: number, objectif: number): Row[] {
  ...
  for (let t = 1; t <= horizon; t++) {
    ...
    debut = fin; // convention : début(t+1) = fin(t)
  }
  return rows;
}

export default function Vision2R2Page() {
  ...
  // On récupère les valeurs éventuelles de R1, sinon défauts.
  const enc0 = toInt(sp.get("enc") ?? "0", 0);
  const dec0 = toInt(sp.get("dec") ?? "0", 0);

  const [horizon, setHorizon] = useState(h0);
  const [s0, setS0] = useState(s00);

  // R2 : encaissements/décaissements constants (pas encore d’interprétation)
  const [encaissements, setEncaissements] = useState(enc0);
  const [decaissements, setDecaissements] = useState(dec0);
  ...
  return (
    <main style={{ maxWidth: 980, margin: "48px auto", padding: 16 }}>
      ...
      <h1 style={{ fontSize: 30, marginBottom: 8 }}>Vision 2 — R2</h1>

      <p style={{ marginTop: 0, fontSize: 16, lineHeight: 1.6 }}>
        R2 introduit l’objectif et permet un verdict <em>avec les hypothèses courantes</em> :
        encaissements/décaissements constants sur l’horizon.
        <br />
      ...
      <section style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 18 }}>
        ...
        <label>
          ...
          <input
            value={encaissements}
            onChange={(e) => setEncaissements(toInt(e.target.value, enc0))}
            style={{ width: "100%", padding: 10, borderRadius: 10, border: "1px solid #ccc" }}
          />
        </label>
        ...
        <label>
          ...
          <input
            value={decaissements}
            onChange={(e) => setDecaissements(toInt(e.target.value, dec0))}
            style={{ width: "100%", padding: 10, borderRadius: 10, border: "1px solid #ccc" }}
          />
        </label>
