    ...
    <main className="min-h-screen grid place-items-center p-6">
      <div className="w-full max-w-3xl space-y-10">
        <header className="text-center space-y-1">
          ...
          <p className="text-sm opacity-70">Flux immuable : Projet → Séquence → Phase 0 → … (lecture seule après validation).</p>
        </header>
        ...
        <section className="space-y-3">
          ...
          <div className="grid gap-2">
            <input
              ...
              value={pTitle} maxLength={120}
              onChange={e => setPTitle(e.target.value)}
            />
            <input
              className="border rounded-lg p-2"
              placeholder="Reconnaissance courte (1–2 lignes)"
              value={pTag} maxLength={200}
            ...
            >Créer ce projet (verrouillé)</button>
          </div>
        </section>

        {/* 2) Choisir un Projet existant */}
        <section className="space-y-3">
              ...
              {currentProjectId && (
                <div className="border rounded-lg p-3 space-y-3 bg-gray-50">
                  ...
                  <div className="grid gap-2">
                    ...
                    >Créer une nouvelle séquence (ouvre Phase 0)</button>
                  </div>
                    ...
                    <ul className="list-disc pl-5 text-sm space-y-1">
                      {previousSequences.map(({ seq, status }) => (
                        <li key={seq.id}>
                          <button
                            className="underline"
                            onClick={() => { selectSequence(seq.id); refresh(); openPhase0(); }}
                          >
                            {seq.title} • {status}
                          </button>
                        </li>
